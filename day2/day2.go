...
func StartDay2A(){
  ...
  for _, item := range passwords{
    ...
    compareString := splits[1][1:]
    rules := strings.Split(splits[0], " ")
    ruleLetter := rules[1]
    rulesCount := strings.Split(rules[0], "-")
    firstCount, _ := strconv.ParseInt(rulesCount[0], 10, 64)
    secondCount, _ := strconv.ParseInt(rulesCount[1], 10, 64)
    
    var foundCount int64 = 0
    
    for _, char := range compareString{
      if(string(char) == ruleLetter){
        foundCount++
      }
    }
    
    if(firstCount <= foundCount && foundCount <= secondCount){
      validCount++
    }
  }
...
func StartDay2B(){
  ...
  for _, item := range passwords{
    ...
    if(string(compareString[firstCount]) == ruleLetter && string(compareString[secondCount]) != ruleLetter){
      validCount++
    }
    if(string(compareString[firstCount]) != ruleLetter && string(compareString[secondCount]) == ruleLetter){
      validCount++
